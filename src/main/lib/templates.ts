...
// Pattern: input → decomposition(+KAC) → splitter → researcher (parallel) → merger → output

const deepResearch: WorkflowTemplate = {
  id: "deep-research",
  name: "Deep Research",
  description:
    "Plan a topic via multi-angle decomposition and KAC, investigate each aspect in parallel, synthesize into a comprehensive report",
  category: "research",
  tags: ["research", "planning", "kac", "fan-out", "parallel", "synthesis"],
  workflow: {
    version: 1,
    name: "Deep Research",
    description:
    ...
    nodes: [
      ...
      },
      {
        ...
        config: {
          skillRef: "researcher",
          prompt:
            "Create a decomposition plan for this topic from different lenses before research starts. Return ONLY a JSON array where each aspect has: name, lens, key_question, why_it_matters, priority (high|medium), and query_seeds (3-4 concrete search queries). Cover 4-6 distinct lenses and avoid overlap. In the same output, include Key Assumptions Check (KAC): add 1-2 high-priority assumption-check aspects that explicitly try to disprove critical hidden assumptions.",
        },
      },
      {
        id: "splitter-1",
        ...
        config: {
          strategy:
            "Input is a prepared JSON array of research aspects (including KAC checks). Create one branch per aspect and preserve the full object details in each branch so the downstream researcher has complete context.",
          maxBranches: 8,
        },
      },
      {
        ...
        config: {
          skillRef: "researcher",
          prompt:
            "Research this aspect thoroughly. Find credible sources, key data points, expert opinions, and emerging trends. Prioritize primary sources and recent publications. For KAC/assumption-check aspects, prioritize disconfirming evidence, failure cases, and counter-examples.",
        },
      },
      {
        id: "merger-1",
        ...
        config: {
          strategy: "summarize",
          prompt:
            "Synthesize all research findings into a comprehensive, well-structured report. Include an executive summary, key findings per aspect, KAC outcomes (which assumptions held or failed), cross-cutting themes, and actionable conclusions.",
        },
      },
      {
        id: "output-1",
    ...
    edges: [
      { id: "e-input-decomposition", source: "input-1", target: "decomposition-1", type: "default" },
      { id: "e-decomposition-splitter", source: "decomposition-1", target: "splitter-1", type: "default" },
...
// ── Template 18: Impeccable UI Pipeline ──────────────────
// Pattern: input → audit → normalize → harden → clarify → optimize → polish → eval → output (pass) / normalize (fail)

const impeccableUIPipeline: WorkflowTemplate = {
  id: "impeccable-ui-pipeline",
  name: "Impeccable UI Pipeline",
  description:
    "Audit a frontend feature, then systematically normalize, harden, clarify, optimize, and polish it with quality-gate evaluation loop",
  category: "code",
  tags: ["ui", "design", "audit", "polish", "quality", "impeccable", "a11y", "performance"],
  workflow: {
    version: 1,
    name: "Impeccable UI Pipeline",
    description:
    ...
    nodes: [
      ...
      },
      {
        ...
        config: {
          ...
          prompt:
            "Run a comprehensive UI quality audit. Check: accessibility (contrast, ARIA, keyboard, semantics), performance (layout thrashing, expensive animations, bundle), theming (hard-coded colors, dark mode), responsive (fixed widths, touch targets, overflow), and anti-patterns (AI slop tells). Output a structured report with severity ratings. DO NOT fix anything — document issues only.",
        },
      },
      {
        id: "normalize-1",
        ...
        config: {
          ...
          prompt:
            "Using the audit report, systematically align this feature with the project's design system. Replace hard-coded values with design tokens, swap custom components for design system equivalents, fix typography/color/spacing inconsistencies, and ensure motion/interaction patterns match established conventions.",
        },
      },
      {
        id: "harden-1",
        ...
        config: {
          ...
          prompt:
            "Strengthen the interface against real-world usage. Handle extreme inputs (long text, empty, emoji, RTL, CJK), network errors (offline, timeout, 4xx/5xx), missing data (empty states, loading states), concurrent operations (double-submit), and ensure keyboard navigation, screen reader support, and prefers-reduced-motion.",
        },
      },
      {
        id: "clarify-1",
        ...
        config: {
          ...
          prompt:
            "Improve all interface text for clarity. Fix error messages (explain + suggest fix), form labels (specific, not generic), button text (verb + noun), empty states (guide to action), help text (add value, not repeat label), and confirmation dialogs (state consequences). Be concise, active voice, human.",
        },
      },
      {
        id: "optimize-1",
        ...
        config: {
          ...
          prompt:
            "Fix performance issues found in audit. Optimize images (modern formats, lazy loading, srcset), reduce JS bundle (code split, tree shake, lazy imports), improve CSS (remove unused, critical inline), fix rendering (avoid layout thrashing, use transform/opacity for animations, virtual scroll for long lists), and ensure Core Web Vitals targets (LCP < 2.5s, INP < 200ms, CLS < 0.1).",
        },
      },
      {
        id: "polish-1",
        ...
        config: {
          ...
          prompt:
            "Final quality pass. Check: pixel-perfect alignment to grid, consistent spacing with design tokens, typography hierarchy, ALL interaction states (default/hover/focus/active/disabled/loading/error/success), smooth transitions (150-300ms, ease-out-quart), consistent copy/capitalization, icon consistency, form labels, error/loading/empty states, touch targets (44px), WCAG contrast, keyboard navigation, no console errors, no layout shift, clean code (no TODOs, console.logs, commented code).",
        },
      },
      {
        id: "evaluator-1",
        ...
        config: {
          criteria:
            "Evaluate the refined UI across 6 dimensions (score 1-10 each): 1) A11y — WCAG AA compliance, keyboard nav, screen reader support; 2) Design system — token usage, component consistency; 3) Copy clarity — error messages, labels, empty states; 4) Performance — no layout thrashing, optimized assets, smooth animations; 5) Interaction completeness — all states implemented; 6) Polish — alignment, spacing, transitions, code cleanliness. Provide specific fix instructions for any dimension below 7.",
          threshold: 7,
          maxRetries: 2,
    ...
    edges: [
      { id: "e-input-audit", source: "input-1", target: "audit-1", type: "default" },
      { id: "e-audit-normalize", source: "audit-1", target: "normalize-1", type: "default" },
      { id: "e-normalize-harden", source: "normalize-1", target: "harden-1", type: "default" },
      { id: "e-harden-clarify", source: "harden-1", target: "clarify-1", type: "default" },
      { id: "e-clarify-optimize", source: "clarify-1", target: "optimize-1", type: "default" },
      { id: "e-optimize-polish", source: "optimize-1", target: "polish-1", type: "default" },
      { id: "e-polish-evaluator", source: "polish-1", target: "evaluator-1", type: "default" },
      { id: "e-evaluator-output", source: "evaluator-1", target: "output-1", type: "pass" },
      { id: "e-evaluator-normalize", source: "evaluator-1", target: "normalize-1", type: "fail" },
    ],
  },
}
